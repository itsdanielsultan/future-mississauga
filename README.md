...
All application and image URLs are relative, so the same files work at `https://itsdanielsultan.github.io/future-mississauga/` or a custom domain. The GitHub Pages address is the initial publication target. A custom domain is optional and must be registered and confirmed owned before it is configured. When that is ready, add its `public/CNAME`, publish the static branch and configure the matching Pages/DNS settings. The repository contains no DNS credentials.

The guarded exporter defaults publication metadata to the GitHub project address. After domain ownership is confirmed, re-export with `--site-origin https://futuremississauga.ca/` before configuring the custom domain. Canonical links, social preview metadata, robots and sitemap follow the selected origin; research links remain unchanged.
