...
An independent visual atlas by Daniel Sultan: original 4K pictures, native 4:5 portrait images, a research register, and an interactive city model with existing GO rail corridors, the Mississauga Transitway and the Hazel McCallion Line. This is the community revision of 26 September 2026.
...
Application and displayed-image URLs are relative. The 33 full native portrait downloads are individual PNG assets in the same repository's `community-images-2026-09-26` GitHub Release; their exact SHA-256 digests are recorded in `public/data/social-pictures.json`. This keeps the Pages site below its hosting-size limit while retaining the original PNG bytes. Its half-size portrait previews are bundled locally in `public/gallery/social/previews/`. No model archives are distributed with the image release.

The intended primary address is `https://futuremississauga.com/`, with `futuremississauga.ca` redirecting to `.com` while preserving paths and queries. `https://itsdanielsultan.github.io/future-mississauga/` remains the publishing fallback. Domain ownership, DNS, redirects and HTTPS must be verified separately before claiming the custom addresses are live. Configure only the `.com` primary in `public/CNAME` and GitHub Pages; the `.ca` redirect is a separate hosting/DNS task. The repository contains no DNS credentials.
...
The exporter enforces a conservative published-size limit below 1,000,000,000 bytes and a per-file limit below 100,000,000 bytes. Native Blender scenes, immutable backups and the untouched municipal archive remain in the local deliverable collection and are not published here. Direct model-download controls have been removed. The interactive viewer still transfers display geometry to the browser; this is not a copy-protection mechanism.
